...
export function Majors() {
  ...
  useEffect(() => { loadMajors(); }, []);

  const loadMajors = async () => {
    try {
      setMajors(await api.majors.list());
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingMajor) {
        await api.majors.update(editingMajor.id, name);
      } else {
        await api.majors.create(name);
      }
      await loadMajors();
      handleCloseModal();
    } catch (err) {
  ...
  const handleDelete = async (id: number, majorName: string) => {
    if (!confirm(`ยืนยันการลบสาขาวิชา "${majorName}"?`)) return;
    try {
      await api.majors.delete(id);
      await loadMajors();
    } catch (err) {
              ...
              {majors.length === 0 ? (
                ...
                <tr key={major.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right font-medium">
                    <button onClick={() => handleOpenModal(major)} className="text-blue-600 hover:text-blue-800 font-bold text-xs mr-3">แก้ไข</button>
                    <button onClick={() => handleDelete(major.id, major.name)} className="text-slate-400 hover:text-slate-600 font-bold text-xs">ลบ</button>
                  </td>
                </tr>
              ))}
            </tbody>
