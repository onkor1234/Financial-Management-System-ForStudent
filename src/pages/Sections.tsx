...
import { db, Section } from '../lib/mockDb';
...
export function Sections() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (editingSection) {
      const idx = db.sections.findIndex(s => s.id === editingSection.id);
      if (idx !== -1) {
        // Also update students and payment requests that use this section? 
        // For simplicity, we just update the section name. In a real app we might update related records or refer by ID.
        // Let's update related records for consistency in mockDB.
        const oldName = db.sections[idx].name;
        db.sections[idx] = { ...editingSection, name: name };
        
        db.students.forEach(s => {
          if (s.section === oldName) s.section = name;
        });
        db.payment_requests.forEach(pr => {
          if (pr.target_sections.includes(oldName)) {
            pr.target_sections = pr.target_sections.map(s => s === oldName ? name : s);
          }
        });
      }
    } else {
      const newId = db.sections.length > 0 ? Math.max(...db.sections.map(s => s.id)) + 1 : 1;
      db.sections.push({
        id: newId,
        name
      });
                ...
                {sections.length === 0 ? (
                  ...
                  <tr key={section.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right font-medium">
                      <button onClick={() => handleOpenModal(section)} className="text-blue-600 hover:text-blue-800 font-bold text-xs mr-3">
                        แก้ไข
                      </button>
                      <button onClick={() => handleDelete(section.id, section.name)} className="text-slate-400 hover:text-slate-600 font-bold text-xs">
                        ลบ
                      </button>
                    </td>
                  </tr>
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto">
          <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div className="relative inline-block w-full max-w-md p-6 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-xl border border-slate-200">
              <h3 className="text-lg font-bold text-slate-900 mb-4">{editingSection ? 'แก้ไขกลุ่มเรียน' : 'เพิ่มกลุ่มเรียนใหม่'}</h3>
